...
func setOwnerAddress() common.Address {
	...
	if owner == "" {
		...
		if privKeyString == "" {
			// No hardcoded fallback key — generate your own local-devnet key
			// and set DEPLOYMENT_PRIVATE_KEY (or INITIAL_OWNER) instead.
			logger.Fatalf("DEPLOYMENT_PRIVATE_KEY (or INITIAL_OWNER) not set")
		} else {
